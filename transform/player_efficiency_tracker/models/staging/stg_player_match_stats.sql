...
select
    ...
    (player_entry -> 'statistics' -> 0 -> 'goals' ->> 'total')::int         as goals_total,
    (player_entry -> 'statistics' -> 0 -> 'goals' ->> 'assists')::int       as assists,
    (player_entry -> 'statistics' -> 0 -> 'cards' ->> 'yellow')::int        as yellow_cards,
    (player_entry -> 'statistics' -> 0 -> 'cards' ->> 'red')::int           as red_cards,
    (player_entry -> 'statistics' -> 0 -> 'fouls' ->> 'committed')::int     as fouls_committed
from player_entries